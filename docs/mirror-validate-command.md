...
### With Static Parts Filtering

```bash
# First, identify static parts
npx elephant-cli identify-static-parts \
  --input-zip properties.zip \
  --output static-parts.csv

# Then, use static parts filtering in mirror-validate
npx elephant-cli mirror-validate \
  --prepare-zip path/to/prepare-output.zip \
  --transform-zip path/to/transform-output.zip \
...
| `--static-parts` | No | Path to CSV file with static DOM selectors to exclude from analysis (generated by `identify-static-parts` command) |
...
### 4. Static Parts Filtering (Optional)

**Files:** `src/utils/static-parts-filter.ts`, `src/services/static-parts-identifier.service.ts`

**Purpose:**
Improves validation accuracy by filtering out boilerplate/template content (navigation, headers, footers) that appears in raw HTML but not in transformed data. This reduces false negatives in completeness scoring.

**How it works:**
1. Use `identify-static-parts` command to analyze multiple HTML files and identify repeated DOM elements
2. Save the resulting CSS selectors to a CSV file
3. Pass the CSV to `mirror-validate` via `--static-parts` option
4. Static DOM parts are removed from raw HTML before NER processing
5. Only dynamic, property-specific content is compared

**Benefits:**
- **Higher accuracy**: Eliminates template content that inflates raw entity counts
- **Better coverage scores**: Focuses comparison on actual data transformations
- **Fewer false negatives**: Reduces unmatched entities from navigation/UI elements

**Utilities:**
- `parseStaticPartsCsv(csvPath)`: Parses CSV file, returns array of CSS selectors
- `removeStaticParts(html, selectors)`: Filters HTML using querySelectorAll + DOM removal
