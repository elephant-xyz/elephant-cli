...
- [Scrape Permit Data](#scrape-permit-data)
...
Use `scrape-permits` to download building permit HTML files for a specific property from a county permit system.

```bash
elephant-cli scrape-permits 00424631070070270 \
  --output-dir ./permits-output \
  --no-headless \
  --zip
```
...
- Launches a browser, navigates to the county permit portal, and waits for permit data to load.
- Saves the permits list page as HTML.
- Clicks through each individual permit to its detail page, expands all collapsible sections, and saves the full HTML.
- Generates a `manifest.json` summarizing the scrape (PCN, file list, errors, timestamp).
- Optionally bundles everything into a ZIP file.

**Inputs**

- Parcel Control Number (PCN) — the county property identifier (e.g., `00424631070070270`).

**Output**

- A directory containing one HTML file per permit detail page, plus the permits list page and a `manifest.json`.
- With `--zip`, an additional `{pcn}_permits.zip` file alongside the output directory.

**Options**

| Option               | Description                                                                 | Default       |
| -------------------- | --------------------------------------------------------------------------- | ------------- |
| `--output-dir <path>` | Directory to save downloaded HTML files.                                   | *(required)*  |
| `--county <name>`    | County scraper to use. Currently supported: `palm-beach`.                   | `palm-beach`  |
| `--no-headless`      | Show the browser window during scraping (recommended for reCAPTCHA sites).  | headless      |
| `--proxy <url>`      | Proxy URL in `username:password@ip:port` format.                            | none          |
| `--zip`              | Bundle output into a ZIP file alongside the output directory.               | `false`       |

**Notes**

- The Palm Beach County site uses reCAPTCHA v3 which blocks headless browsers. Use `--no-headless` for reliable results.
- Properties with no permits on file will produce only the list page and an empty manifest.
- If a single permit fails to scrape, the command logs the error and continues with remaining permits.
