...
describe('JsonValidatorService - Currency Format', () => {
  ...
  describe('Currency Format with number type', () => {
    ...
    it('should validate valid currency values', async () => {
      const validValues = [
        100, 100.0, 100.0, 100.5, 100.5, 100.99, 1000, 1000000, 0.5, 0.01,
        999999999.99, 1.1, 1.11, 42.42, 0.1, 0.99,
      ];
    ...
    it('should reject non-numeric values', async () => {
      ...
      for (const value of invalidValues) {
        const result = await jsonValidator.validate(value, currencySchema);
        expect(result.valid, `Should reject non-numeric value: ${value}`).toBe(
          false
        );
      }
    });
    ...
    it('should reject special numeric values', async () => {
      ...
      for (const value of invalidValues) {
        const result = await jsonValidator.validate(value, currencySchema);
        expect(result.valid, `Should reject special value: ${value}`).toBe(
          false
        );
      }
    });
    ...
    it('should reject negative numbers', async () => {
      ...
      for (const test of values) {
        ...
        expect(
          result.valid,
          `Value ${test.value} should be ${test.expected ? 'valid' : 'invalid'}`
        ).toBe(test.expected);
      }
    });
    ...
    it('should handle edge cases', async () => {
      ...
      for (const test of edgeCases) {
        ...
        expect(
          result.valid,
          `Edge case ${test.value} should be ${test.expected ? 'valid' : 'invalid'}`
        ).toBe(test.expected);
      }
    });
  });
  ...
  describe('Schema validation with currency format', () => {
    ...
    it('should work in arrays', async () => {
      ...
      const validData = [10.0, 20.5, 30.99, 40];
      const result = await jsonValidator.validate(validData, schema);
      expect(result.valid).toBe(true);

      const invalidData = [10.0, 20.555, 30.99]; // One has 3 decimal places
