...
describe('JsonCanonicalizerService', () => {
  ...
  describe('canonicalizeBatch', () => {
    it('should canonicalize an array of JSON objects', () => {
      const arr = [
        { b: 1, a: 2 },
        { d: 3, c: 4 },
      ];
      const expected = ['{"a":2,"b":1}', '{"c":4,"d":3}'];
      const results = service.canonicalizeBatch(arr);
      expect(results).toEqual(expected);
    });

    it('should handle empty array for batch', () => {
      const results = service.canonicalizeBatch([]);
  ...
  describe('areEquivalent', () => {
    it('should return true for equivalent JSON objects', () => {
      expect(service.areEquivalent({ a: 1, b: 2 }, { b: 2, a: 1 })).toBe(
        true
      );
    });

    it('should return false for non-equivalent JSON objects', () => {
      expect(service.areEquivalent({ a: 1, b: 2 }, { a: 1, b: 3 })).toBe(
        false
      );
    });
