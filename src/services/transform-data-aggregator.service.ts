...
export class TransformDataAggregatorService {
  async aggregateTransformOutput(
    ...
    const relationshipFiles = jsonFiles.filter((f) =>
      f.toLowerCase().includes('relationship')
    );
    ...
    for (const relFile of relationshipFiles) {
      const relPath = path.join(transformOutputDir, relFile);
  ...
  convertAggregatedDataToText(
    aggregatedData: Record<string, JsonObject[]>
  ): string {
