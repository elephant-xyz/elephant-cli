...
export function registerTransformCommand(program: Command) {
  program
    ...
    .option(
      ...
      'Force property improvement mode (creates Property Improvement data group)'
    )
    .option(
      '--input-zip <path>',
...
async function handleScriptsMode(options: TransformCommandOptions) {
  ...
  let isPropertyImprovementMode = false;
    if (resolvedScriptsZip) {
      ...
      isPropertyImprovementMode = await handleCountyTransform(scriptsDir, tempRoot, options.propertyImprovement);
...
async function handleCountyTransform(scriptsDir: string, tempRoot: string, forcePropertyImprovement = false): Promise<boolean> {
  ...
  // 3) Check for relationship files generated by the script
  if (hasPropertyImprovementData) {
    // Look for relationship files that might have been generated by the script
    const outputFiles = await fs.readdir(path.join(tempRoot, OUTPUT_DIR));
    for (const file of outputFiles) {
      if (file === 'property_improvement_to_company.json' || file === 'company_to_communication.json') {
  ...
  console.log('All relationship files:', relationshipFiles);
  console.log('hasPropertyImprovementData:', hasPropertyImprovementData);
  ...
  if (hasPropertyImprovementData) {
    ...
    console.log('Property improvement relationships being passed:', propertyImprovementRelationships);
    const propertyImprovementDataGroup = createPropertyImprovementDataGroup(propertyImprovementRelationships);
    console.log('Created property improvement data group:', JSON.stringify(propertyImprovementDataGroup, null, 2));
