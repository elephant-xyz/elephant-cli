...
async function extractSourceData(prepareDir: string): Promise<string> {
  ...
  if (/\.html?$/i.test(htmlFile)) {
    const cleaned = await cleanHtml(rawContent);
...
async function mirrorValidate(options: MirrorValidateOptions): Promise<void> {
  ...
  try {
    console.log(chalk.gray('[1/6] Extracting prepare output...'));
    ...
    console.log(chalk.gray('[2/6] Extracting transform output...'));
    ...
    console.log(chalk.gray('[3/6] Extracting entities from raw data...'));
    const rawText = await extractSourceData(prepareTempRoot);
